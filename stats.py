...
class GoogleTrendAnalyzer:
    def __init__(self, filename):
        ...
        with open(filename, 'r') as f:
            ...
            self.headers = reader.__next__()  # read one line which contains the column headers
            for row in reader:
                self.table.append(row)  # add each csv row to the table

    def most_popular(self):
        """
        Return the string of the header of the column for the item with the highest popularity
        :return:
        """
        max_col_number = -1
        max_value = float("-inf")
        for row in self.table:
            # for data, col_number in enumerate(row):
